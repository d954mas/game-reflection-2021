...
static int check_fill(lua_State* L){
    read_and_validate_buffer_info(L, 1);
     uint32_t *pixels = (uint32_t *)buffer_info.bytes;
    int free_zone = 0;
    int fill_zone = 0;
    uint32_t free_color = 0xFFFFFFFF;
    uint32_t fill_color = 0xFF0000FF;
    for(int i = 0;i<buffer_info.width*buffer_info.height;i++){
        uint32_t color = pixels[i];
        if(color == fill_color){
            fill_zone++;
        }else if(color == free_color){
            free_zone++;
        }
    }
    lua_pushnumber(L, free_zone);
    lua_pushnumber(L, fill_zone);
    return 2;
}
