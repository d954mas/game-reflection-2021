...
function Lbl:set_value(value, force)
	...
	self:force_finish()
	self.animation.tween = ACTIONS.Tween { object = self.animation, property = "value", from = { value = self.value }, to = { value = value },
										   time = self.animation_config.time, easing = self.animation_config.easing }
	self.value = value
